...
export async function weappLogin(req: Request, res: Response, next: NextFunction) {
    ...
    if (openid && session_key) {
        let openid_str = openid.substring(openid.length - 8);
        let time_str = new Date().getTime().toString().substring(5);
...
export async function weappSign(req: Request, res: Response, next: NextFunction) {
    ...
    if (!(weappUserInfo && weappUserInfo.unionId)) return res.sendErr('获取信息异常');

    let options = {
        unionid: weappUserInfo.unionId,
    ...
    await redis.set(`${wxconfig.session_key_pre}:user:${user.id}`, session_key, 86400);
