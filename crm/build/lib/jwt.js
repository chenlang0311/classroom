...
function jwtEncode(user) {
    ...
    let token = jwt.sign(payload, new Buffer(config_1.config.jwt.secret, 'base64'), { expiresIn: config_1.config.jwt.expiresIn });
    return token;
}
...
function jwtDecode(token) {
    ...
    let decoded;
    try {
        decoded = jwt.verify(token, new Buffer(config_1.config.jwt.secret, 'base64'));
    }
    catch (err) {
...
function jwtVerify(req, res, next) {
    ...
    try {
        jwt.verify(token, new Buffer(config_1.config.jwt.secret, 'base64'));
    }
    catch (err) {
        logger.error(err.stack);
        return res.sendErr('无效的token');
...
function jwtMiddle(req, res, next) {
    ...
    if (token) {
        try {
            req.jwtAccessToken = jwt.verify(token, new Buffer(config_1.config.jwt.secret, 'base64'));
        }
        catch (err) {
