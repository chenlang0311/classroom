...
function cloneChildToSelf(obj, arrKey) {
    if (!isObject(obj))
        return obj;
    ...
    for (let key in data) {
        ...
        if (isObject(val)) {
            ...
            try {
                val.forEach((el) => {
                    el = el.get();
                    if (isObject(el) && el[arrKey])
                        data[el[arrKey]] = cloneObj(el);
                });
                if (arrKey)
                    delete data[key];
            }
...
exports.cloneChildToSelf = cloneChildToSelf;
...
function ObjToStringDictSort(obj) {
    if (!isObject(obj))
        return obj;
