...
export function cloneChildToSelf(obj: any, arrKey?: string) {
    if (!isObject(obj)) return obj;
    let data: any = cloneObj(obj);
    ...
    for (let key in data) {
        ...
        if (isObject(val)) {
            try {
                val = val.get();
                for (let kk in val) { if (!data[kk]) data[kk] = val[kk]; }
                delete data[key];
            } catch (e) { }
        } else if (isArray(val)) {
            try {
                val.forEach((el: any) => {
                    el = el.get();
                    if (isObject(el) && el[arrKey]) data[el[arrKey]] = cloneObj(el);
                });
                if (arrKey) delete data[key];
            } catch (e) { }
        }
    }
...
export function ObjToStringDictSort(obj: any) {
    if (!isObject(obj)) return obj;
